...
> - **UKUR**（by RICCIONE REKA）— 户型图量尺：上传户型图，标定一个已知尺寸，其余长度按比例量出来，自动汇总橱柜延尺与预估报价。代码在 [`/measure`](./measure)，部署指南见 [`measure/DEPLOY.md`](./measure/DEPLOY.md)。
...
measure/                  # 独立的 UKUR 户型图量尺 app（单独部署）
```
